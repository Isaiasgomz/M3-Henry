...
args.forEach(function(arg){
 ...
 async function problemB () {
  ...
  // AsyncAwait version
   ...
   const p1 =  await promisifiedReadFile('poem-one/stanza-02.txt')
     blue(p1)
  

  const p2 = await promisifiedReadFile('poem-one/stanza-03.txt')
     blue(p2)
...
async function problemD () {
  ...
  try {
    const p1 = await promisifiedReadFile('poem-one/wrong-file-name.txt')
      blue(p1) 
  } catch (error) {
    magenta(error)
  }
...
async function problemE () {
  ...
  try {
    const p1 =  await promisifiedReadFile('poem-one/stanza-03.txt')
      blue(p1)
    
    const p2 =  await promisifiedReadFile('poem-one/wrong-file-name.txt')
      blue(p2)
